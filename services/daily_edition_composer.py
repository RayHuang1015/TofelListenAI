    ...
    def _get_available_content(self, target_date: date) -> List[ContentSource]:
        ...
        if total_available_duration < self.MIN_DURATION:
            self.logger.info(
                ...
                f"Generating additional content via HistoricalNewsGenerator."
            )
            
            # Generate additional content using HistoricalNewsGenerator
            ...
            if new_content_items:
                try:
                    db.session.commit()
                    self.logger.info(f"Generated and saved {len(new_content_items)} new content items")
                    content.extend(new_content_items)
                except Exception as e:
